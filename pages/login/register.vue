...
<!-- 			<wButton
			...
			<!-- 底部信息 -->
			<!-- 不知道怎么返回到上一页 -->
			 <!-- <navigator url="index" open-type="navigateBack">返回</navigator> -->
...
<script >
	...
	import {getAndroidCheckCodeReq} from '../../service/genService.js'
	import {AndroidRegisterReq} from '../../service/userService.js'
	
	export default {
		data() {
			// const role_id = ref('乘客');
			return {
				//logo图片 base64
		...
		methods: {
		    ...
		    startReg() {
				...
				AndroidRegisterReq(this.name,this.phone,this.verCode,this.password,this.role_id).then((res)=>{
					if(res.data.success){
						uni.showToast({
							title:'用户注册成功'
						})
