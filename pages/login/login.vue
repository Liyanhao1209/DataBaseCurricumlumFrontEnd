			...
			<view class="main">
				...
				<view class="uni-list-cell">
					...
					<view class="uni-list-cell-db">
						<picker @change="bindPickerChange" :value="roles_index" :range="roles">
							<view class="uni-input">{{roles[roles_index]}}</view>
						</picker>
					</view>
				</view>
					<!-- <u-radio-group v-model="role_id" @change="radioGroupChange">
						<u-radio name="乘客">乘客</u-radio>
						<u-radio name="司机">司机</u-radio>
					</u-radio-group> -->
			</view>
...
<script>
	...
	import {getCheckCodeReq,getAndroidCheckCodeReq} from '../../service/genService.js'
	...
	export default {
		...
		mounted() {
			_this= this;
			this.role_id = this.roles_index
			// uni.request({
			// 	url:'http://192.168.1.11:8081/api/user/getTest',
			// 	method:'GET',
			// 	header: {
		...
		methods: {
			...
			bindPickerChange: function(e){
				this.roles_index = e.detail.value
				if(this.roles_index===1){
					this.role_id = 1
				}
				else if(this.roles_index===2){
					this.role_id = 2
				}
			},
			...
			// radioGroupChange(e){
			// 	console.log(e);
			// 	if(this.props.name=="乘客"){
			// 		this.role_id=1
			// 	}
			// 	else if(this.props.name=="司机"){
			// 		this.role_id=2
			// 	}
			// },
			getCheckCode(){
				// console.log(uni.getStorageSync("loggedIn"))
				...
				getAndroidCheckCodeReq(this.phone).then((res)=>{
					if(res.data.success){
						uni.showToast({
							title:"验证码已发送"
						})
		    ...
		    startLogin(e){
		        ...
		        if (this.password.length < 6&&this.code.length==0) {
		            uni.showToast({
		                icon: 'none',
						position: 'bottom',
		                title: '密码至少为6位'
				...
				var flag = this.phone!=null && (this.checkCode!=null||this.password!=null);
				if(flag){
				this.$store.dispatch("AndroidLoginCommit",
				{   
					phone:this.phone,
					...
					uni.showToast({
						title:'验证码和密码至少有一个需要输入，必须填写手机号,且必须选择用户身份'
					})
				}
